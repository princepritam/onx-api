...
def update_session(action=None):
    try:
        ...
        if action == "start" and session_.status == 'accepted':
            ...
            Activity(user_id= session_.members[0], session_id=str(session_._id), is_dynamic= False, content= "You successfully started a session", created_at= datetime.datetime.now().isoformat()).save()
            # code.interact(local=dict(globals(), **locals()))
            ...
            Activity(user_id= session_.members[0], session_id=str(session_._id), is_dynamic= False, content= "Successfully Ended session", created_at= datetime.datetime.now().isoformat()).save()
